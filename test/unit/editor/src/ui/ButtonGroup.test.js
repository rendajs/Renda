	...
	fn() {
		runWithDom(() => {
			...
			function testClasses(className, containsState) {
				assertEquals(button1.el.classList.contains(className), containsState[0]);
				assertEquals(button2.el.classList.contains(className), containsState[1]);
				assertEquals(button3.el.classList.contains(className), containsState[2]);
			}
