	...
	await testContext.step({
		...
		fn: async () => {
			await page.waitForFunction(() => {
				...
				return globalThis.editor.popoverManager.currentContextMenu;
			});
			for (let i = 0; i < menuPath.length; i++) {
				...
				const jsHandle = await page.evaluateHandle(async (itemName, expectedSubmenuCount, menuPath) => {
					if (!globalThis.editor) throw new Error("Editor instance does not exist");
					if (!globalThis.editor.popoverManager.currentContextMenu) throw new Error("Context menu no longer exists");
					...
					// last submenu.
					let submenuCount = 0;
					let currentMenu = globalThis.editor.popoverManager.currentContextMenu;
					while (true) {
